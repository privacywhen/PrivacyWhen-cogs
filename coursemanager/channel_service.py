...
class ChannelService:
    """Service for managing course channels and related operations."""
    ...
    async def set_default_category(self, ctx: discord.ext.commands.Context, category_name: str) -> None:
        """
    ...
    async def create_channel(
        self, ctx: discord.ext.commands.Context, channel_name: str, category: Optional[discord.CategoryChannel] = None
    ) -> None:
        """
        Create a new text channel within the specified or default category.

        Args:
            ctx (discord.ext.commands.Context): The command context.
            channel_name (str): The name of the channel to create.
            category (Optional[discord.CategoryChannel]): The category in which to create the channel. Defaults to None.
        """
        ...
        if category is None:
            await ctx.send(error("I do not have permission to create the default category."))
            return
        try:
            channel = await guild.create_text_channel(channel_name, category=category)
            await ctx.send(f"Channel {channel.mention} created in category **{category.name}**.")
        except discord.Forbidden:
            await ctx.send(error("I do not have permission to create a channel in that category."))

    async def channel_prune_helper(
        self, guild: discord.Guild, channel: discord.TextChannel, prune_threshold: timedelta
    ) -> None:
        """
        Helper method to prune a channel if inactive beyond the threshold.

        Args:
            guild (discord.Guild): The guild in which the channel exists.
            channel (discord.TextChannel): The text channel to evaluate.
            prune_threshold (timedelta): The inactivity threshold for pruning.
        """
        ...
        if last_activity is None:
            ...
            log.debug(f"No non-bot messages found in {channel.name}. Using channel.created_at: {last_activity}")
        ...
        if inactivity_duration > prune_threshold:
            ...
            try:
                ...
                log.exception(f"Failed to delete channel '{channel.name}' in guild '{guild.name}': {e}")

    async def auto_channel_prune(self) -> None:
        """
        Periodically check and prune inactive channels based on the configured threshold.
        """
        ...
        try:
            while not self.bot.is_closed():
                log.debug(f"Auto-channel-prune cycle started at {datetime.now(timezone.utc)}")
                ...
                log.debug(f"Auto-channel-prune cycle complete. Sleeping for {prune_interval} seconds.")
